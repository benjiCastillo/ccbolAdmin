...
--
-- Servidor: localhost
-- Tiempo de generación: 08-09-2017 a las 01:58:46
	...
	IF(SELECT EXISTS(SELECT * FROM admin WHERE id=_id_admin1 OR id=_id_admin2))THEN
		...
		WHILE cont<=_limit AND cont<=10 DO
            ...
            IF(SELECT EXISTS(SELECT * FROM student WHERE id_user=_id))THEN
				SET _info=(SELECT college FROM student WHERE id_user=_id);
		...
		select 'not' as error, id, name, last_name, cargo, info from user_temp;
